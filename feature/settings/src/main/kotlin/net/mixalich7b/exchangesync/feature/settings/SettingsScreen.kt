...
private fun SynchronizationSection(
    ...
    if (uiState.isDisableSyncVisible) {
        var isDisableConfirmationVisible by rememberSaveable { mutableStateOf(false) }

        Spacer(modifier = Modifier.height(8.dp))
        OutlinedButton(
            onClick = { isDisableConfirmationVisible = true },
            enabled = uiState.isDisableSyncEnabled,
            modifier = Modifier.fillMaxWidth(),
        ) {
            Text(stringResource(R.string.sync_disable))
        }
        if (isDisableConfirmationVisible) {
            AlertDialog(
                onDismissRequest = { isDisableConfirmationVisible = false },
                title = { Text(stringResource(R.string.sync_disable_confirmation_title)) },
                text = { Text(stringResource(R.string.sync_disable_confirmation_message)) },
                confirmButton = {
                    TextButton(
                        onClick = {
                            isDisableConfirmationVisible = false
                            onDisableSynchronization()
                        },
                    ) {
                        Text(
                            text = stringResource(R.string.sync_disable_confirmation_confirm),
                            color = MaterialTheme.colorScheme.error,
                        )
                    }
                },
                dismissButton = {
                    TextButton(onClick = { isDisableConfirmationVisible = false }) {
                        Text(stringResource(R.string.sync_disable_confirmation_cancel))
                    }
                },
            )
        }
